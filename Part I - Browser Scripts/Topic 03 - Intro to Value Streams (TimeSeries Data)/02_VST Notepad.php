    ...
    var app = createApp({
        ...
        methods: {
            ...
            GenerateSeries: async function(){
                this.newSeries = [];
                let newSeries = [];
                let timestamp = this.newDateTimeMoment.format('YYYY-MM-DD HH:mm:SS');
                for(let i=0; i<this.newSeriesCount; i++){
                    newSeries.push({
                        value: i,
                        status: 0,
                        timestamp: i==0 ? timestamp : moment(newSeries[i-1].timestamp).tz(this.activeTimeZone.value).add(this.newSeriesInterval).format('YYYY-MM-DD HH:mm:SS')
                    });
                }
